...
class PUGeo(nn.Module):
    def __init__(self, knn=20, fd=128, train_up_ratio=16):
        ...
        self.dgcnn_conv1 = nn.Sequential(
            nn.Conv2d(6, fd, kernel_size=1), nn.BatchNorm2d(fd), nn.LeakyReLU(negative_slope=0.2)
        )
        self.dgcnn_conv2 = nn.Sequential(
            nn.Conv2d(fd, fd, kernel_size=1), nn.BatchNorm2d(fd), nn.LeakyReLU(negative_slope=0.2)
        )
        self.dgcnn_conv3 = nn.Sequential(
            nn.Conv1d(fd + fd, fd, kernel_size=1), nn.BatchNorm1d(fd), nn.LeakyReLU(negative_slope=0.2)
        )
        self.dgcnn_conv4 = nn.Sequential(
            nn.Conv2d(fd + fd, fd, kernel_size=1), nn.BatchNorm2d(fd), nn.LeakyReLU(negative_slope=0.2)
        )
        self.dgcnn_conv5 = nn.Sequential(
            nn.Conv1d(fd + fd, fd, kernel_size=1), nn.BatchNorm1d(fd), nn.LeakyReLU(negative_slope=0.2)
        )
        self.dgcnn_conv6 = nn.Sequential(
            nn.Conv2d(fd + fd, fd, kernel_size=1), nn.BatchNorm2d(fd), nn.LeakyReLU(negative_slope=0.2)
        )
        self.dgcnn_conv7 = nn.Sequential(
            nn.Conv1d(fd + fd, fd, kernel_size=1), nn.BatchNorm1d(fd), nn.LeakyReLU(negative_slope=0.2)
        )

        self.uv_2order_coefficient_conv = nn.Sequential(
            nn.Conv1d(fd * 9, 256, kernel_size=1),
            nn.BatchNorm1d(256),
        ...
        uv = uv.unsqueeze(0).repeat(
            grid_size * grid_size, 1, 1
        )  # (grid_size*grid_size,grid_size*grid_size,3)  ,  regard as (B,N,3)
        ...
        uv = utils.gather_operation(grid.transpose(1, 2).contiguous(), index).transpose(1, 2)[
            :, :, 0:2
        ]  # (grid_size*grid_size,up_ratio,2)
    ...
    def forward(self, x, up_ratio=16, poisson=True):
        ...
        sel_uv_index = (
            torch.randint(0, int(up_ratio * up_ratio), size=(int(batch_size * num_point), 1, 1))
            .repeat(1, self.uv_set.size(1), 2)
            .to(concat)
        )

        uv = (
            torch.gather(self.uv_set.to(concat), dim=0, index=sel_uv_index.type(torch.int64))
            .reshape(batch_size, num_point, -1, 2)
            .to(concat)
        )
        ...
        coefficient = (
            self.uv_2order_coefficient_conv(concat).transpose(2, 1).reshape(batch_size, num_point, 6, 3)
...
class UDF(nn.Module):
    ...
    def forward(self, input_dict, query):
        ...
        _, idx, query_knn_pc = pytorch3d.ops.knn_points(
            query, dense_pc, K=self.K, return_nn=True, return_sorted=False
        )  # (B,M,K)    (B,M,K,3)
        ...
        concat_vector1 = torch.cat((query_knn_pc_local, oriented_normal), dim=3).permute(
            0, 3, 1, 2
        )  # (B,M,K,6)->(B,6,M,K)
        ...
        concat_vector2 = torch.cat(
            (concat_vector1, dist.permute(0, 3, 1, 2), feature, patch_feature.repeat(1, 1, 1, self.K)), dim=1
        )  # (B,6+1+128+128,M,K)
