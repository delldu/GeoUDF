...
class mesh_pc_dataset(data.Dataset):
    def __init__(self, data_path, mode="train", min_num_point=3000, max_num_point=48000, num_sample=2048, rot=False):
        ...
        for cat in cat_list:
            ...
            self.data_path = self.data_path + [
                os.path.join(data_path, cat, model)
                for model in models_c
                if os.path.exists(os.path.join(data_path, cat, model))
            ]
    ...
    def rotate_point_cloud_and_gt(self):
        ...
        Ry = np.array([[np.cos(angles), 0, -np.sin(angles)], [0, 1, 0], [np.sin(angles), 0, np.cos(angles)]])
    ...
    def __getitem__(self, idx):
        ...
        idx1 = (
            (np.isnan(closest_points[:, 0]) == False)
            & (np.isnan(closest_points[:, 1]) == False)
            & (np.isnan(closest_points[:, 2]) == False)
            & (np.isinf(closest_points[:, 0]) == False)
            & (np.isinf(closest_points[:, 1]) == False)
