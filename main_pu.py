...
if __name__ == "__main__":
    ...
    dataset = mesh_pc_dataset(
        arg.data_path, mode="train", min_num_point=arg.min_num_point, max_num_point=arg.max_num_point, rot=False
    )
    dataset_test = mesh_pc_dataset(
        arg.data_path, mode="val", min_num_point=arg.min_num_point, max_num_point=arg.max_num_point
    )

    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=arg.batch_size, shuffle=True, drop_last=True, num_workers=16
    )
    dataloader_test = torch.utils.data.DataLoader(
        dataset_test, batch_size=arg.batch_size // 10, shuffle=False, drop_last=False, num_workers=16
    )
    ...
    for epoch in range(arg.max_epoch):
        ...
        with torch.no_grad():
            for data in tqdm(dataloader_test, desc="epoch %d test" % epoch):
                ...
                loss_dense_cd_test = chamfer_distance(
                    gen_dense_xyz.reshape(batch_size, -1, 3), gt_dense_xyz.transpose(1, 2)
                )[0]
